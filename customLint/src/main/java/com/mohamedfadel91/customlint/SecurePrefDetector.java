 ...
 *
 * Detector looking for SecurePref.edit() calls without a corresponding
 * commit() or apply() call
 */
public class SecurePrefDetector extends Detector implements Detector.JavaScanner {
    /**
     * The main issue discovered by this detector
     */
    public static final Issue ISSUE = Issue.create(
            "CommitSecurePrefEdits", //$NON-NLS-1$
            "Missing `commit()` on `SecurePref` editor",
            "After calling `edit()` on a `SecurePref`, you must call `commit()` " +
                    "or `apply()` on the editor to save the results.",
            Category.CORRECTNESS,
            6,
            Severity.WARNING,
            new Implementation(
                    SecurePrefDetector.class,
                    Scope.JAVA_FILE_SCOPE));
    public static final String SECURE_PREF_SHARED_PREFERENCES =
            "com.mohamedfadel91.prefsecure.SecurePref"; //$NON-NLS-1$
    private static final String SECURE_PREF_SHARED_PREFERENCES_EDITOR =
            "com.mohamedfadel91.prefsecure.SecurePref.Editor"; //$NON-NLS-1$

    /**
     * Constructs a new {@link SecurePrefDetector} check
     */
    public SecurePrefDetector() {
    }
    ...
    public void visitMethod(@NonNull JavaContext context, @Nullable AstVisitor visitor,
        ...
        if (resolve instanceof ResolvedMethod) {
            ...
            if (returnType == null ||
                    !returnType.matchesName(SECURE_PREF_SHARED_PREFERENCES_EDITOR)) {
                return;
            }
            verifiedType = true;
        }
        ...
        if (definition == null) {
            if (operand instanceof VariableReference) {
                if (!verifiedType) {
                    ...
                    if (type == null || !type.equals("SecurePref")) { //$NON-NLS-1$
                        return;
                    }
                }
            ...
            if (!verifiedType) {
                ...
                if (!type.endsWith("SecurePref.Editor")) {  //$NON-NLS-1$
                    if (!type.equals("Editor") ||                  //$NON-NLS-1$
                            !LintUtils.isImported(context.getCompilationUnit(),
                                    SECURE_PREF_SHARED_PREFERENCES_EDITOR)) {
                        return;
                    }
                }
            }
        ...
        if (!finder.isCommitCalled()) {
            context.report(ISSUE, method, context.getLocation(node),
                    "`SecurePref.edit()` without a corresponding `commit()` or `apply()` call");
        }
    }
            ...
            if (node == mTarget) {
                ...
                if (isCommit || "apply".equals(name)) { //$NON-NLS-1$ //$NON-NLS-2$
                    ...
                    if (resolved instanceof JavaParser.ResolvedMethod) {
                        ...
                        if (clz.isSubclassOf(SECURE_PREF_SHARED_PREFERENCES_EDITOR, false)
